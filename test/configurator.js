...
contract('Configurator integration test', function (accounts) {
  ...
  before(async function () {
    ...
    const bountyWalletAddress = await configurator.bountyWallet();
    
    token = await Token.at(tokenAddress);
    presale = await Presale.at(presaleAddress);
    mainsale = await Mainsale.at(mainsaleAddress);
    bountywallet = await BountyWallet.at(bountyWalletAddress);
  });
  ...
  it('contracts should have bounty wallet address', async function () {
    const bountywalletOwner = await bountywallet.owner();
    bountywalletOwner.should.bignumber.equal(manager);
  });
  ...
  it('bounty frizze wallet should have firstDate and secondDate time as described in README', async function () {
    const firstDate = await bountywallet.firstDate();
    firstDate.should.bignumber.equal((new Date('01 Dec 2018 00:00:00 GMT')).getTime() / 1000);
    const secondDate = await bountywallet.secondDate();
  ...
  it ('bounty frizze wallet should have master percent as described in README', async function () {
    const masterPercent = await bountywallet.masterPercent();
  ...
  it ('Bounty wallet and founders wallet should be as described in README', async function () {
    const bountyWallet = await bountywallet.wallet();
